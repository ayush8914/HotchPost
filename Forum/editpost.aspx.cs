        ...
        void Updatepost(int pid)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        SqlCommand cmd1;
                        byte[] PostPicBytes = null;
                        if (filePostPic.HasFile)
                        {
                            PostPicBytes = filePostPic.FileBytes;
                            cmd1 = new SqlCommand("UPDATE [dbo].[Post] SET PostTitle='" + tbTitle.Text + "',PostContent='" + tbContent.Text + "',postdate=@postdate,PostPic=@postpic WHERE id =" + pid + ";", con);
                            cmd1.Parameters.AddWithValue("@postdate", DateTime.Now);
                            cmd1.Parameters.AddWithValue("@postpic", PostPicBytes);
                        }
                        else {
                            cmd1 = new SqlCommand("UPDATE [dbo].[Post] SET PostTitle='" + tbTitle.Text + "',PostContent='" + tbContent.Text + "',postdate=@postdate WHERE id =" + pid + ";", con);
                            cmd1.Parameters.AddWithValue("@postdate", DateTime.Now);
                        }
